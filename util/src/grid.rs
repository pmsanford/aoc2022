...
pub trait Grid<T> where T: core::fmt::Debug {
...
pub struct SparseGrid<T> where T: core::fmt::Debug + Default {
    bounds: ((isize, isize), (isize, isize)),
    data: HashMap<(isize, isize), T>,
    renderer: Box<dyn Fn(&T) -> char>
}

impl<T> SparseGrid<T> where T: core::fmt::Debug + Default {
    pub fn new(renderer: Box<dyn Fn(&T) -> char>) -> Self {
        Self { bounds: ((0, 0), (0, 0)), data: HashMap::new(), renderer }
    }
...
impl<T> Grid<T> for SparseGrid<T> where T: core::fmt::Debug + Default {
    fn get(&self, location: (isize, isize)) -> Option<&T> {
    ...
    fn draw(&self, window: ((isize, isize), (isize, isize))) -> Vec<String> {
        ...
        let mut result = vec![];
        let xl = (lower_bound.0..=upper_bound.0).step_by(5).map(|l| format!("{: >3}", l).chars().collect::<Vec<_>>()).collect::<Vec<_>>();
        result.push(format!("    {}", xl.iter().map(|n| n[0].to_string()).collect::<Vec<_>>().join("    ")));
        result.push(format!("    {}", xl.iter().map(|n| n[1].to_string()).collect::<Vec<_>>().join("    ")));
        result.push(format!("    {}", xl.iter().map(|n| n[2].to_string()).collect::<Vec<_>>().join("    ")));
        ...
        for y in lower_bound.1..=upper_bound.1 {
            ...
            result.push(format!("{: >3} {}", y, line.into_iter().collect::<String>()));
        }
